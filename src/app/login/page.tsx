...
import { loginAsRole } from "@/lib/actions/auth";
import { getStore } from "@/lib/store";
import { ROLE_LABELS, type Role } from "@/lib/types";
import { buttonClass } from "@/components/ui/button";

const roles: Role[] = ["buyer", "seller", "verifier", "mediator", "operator"];
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
        ...
        <h1 className="mt-6 text-xl font-semibold tracking-tight">Choose an account</h1>
        <p className="mt-1 text-sm text-zinc-500">Select a workspace identity to continue.</p>

        <div className="mt-6 space-y-2">
          {roles.map((role) => {
            const u = users.find((x) => x.role === role && x.id.includes("-1"));
            return (
              <form key={role} action={loginAsRole.bind(null, role)}>
                <button
                  type="submit"
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold">{u?.name ?? ROLE_LABELS[role]}</p>
