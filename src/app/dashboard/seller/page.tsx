...
export default function SellerDashboard() {
  ...
  const needEvidence = deals.filter((d) =>
  ...
  return (
    ...
    >
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <StatCard label="Active deals" value={String(deals.filter((d) => d.heldEtb > 0).length)} />
      </div>
      {needEvidence.length > 0 && (
        <div className="card p-4 border-amber-200 bg-amber-50/50">
          ...
          <ul className="mt-2 space-y-1 text-sm">
            {needEvidence.map((d) => (
              <li key={d.id}>
                <Link href={`/deals/${d.id}`} className="text-[#0b3d2e] font-medium hover:underline">
                  {d.title}
                </Link>
